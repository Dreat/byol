...
void lval_del(lval *v) {
  ...
  /* Free the memory allocated for the "lval" struct itselt */
  free(v);
}
...
lval *builtin_not(lenv *e, lval *a) { return builtin_logic(e, a, "!"); }
...
lval *builtin_load(lenv *e, lval *a) {
  ...
  mpc_result_t r;
  if (mpc_parse_contents(a->cell[0]->str, Byol, &r)) {
...
lval *builtin_cons(lenv *e, lval *a) {
  ...
  LASSERT(a, a->cell[0]->type == LVAL_NUM,
          "First argument to function 'cons' should be a value");
  LASSERT(a, a->cell[1]->type == LVAL_QEXPR,
          "First argument to function 'cons' should be a value");

  lval *new_head = lval_sexpr();
  lval_add(new_head, a->cell[0]);
  lval *v = builtin_list(e, new_head);

  return lval_join(v, a->cell[1]);
}

lval *qexpr_len(lenv *e, lval *a) {
  long count = 0;
  ...
  for (int i = 0; i < a->count; i++) {
    ...
    case LVAL_SEXPR:
      ...
      break;
    case LVAL_QEXPR:
      count += qexpr_len(e, a->cell[i])->num;
    }
  }
...
lval *builtin_len(lenv *e, lval *a) {
  ...
  for (int i = 0; i < a->cell[0]->count; i++) {
    switch (a->cell[0]->cell[i]->type) {
    case LVAL_NUM:
    case LVAL_SEXPR:
      count += 1;
      break;
    case LVAL_QEXPR:
      count += qexpr_len(e, a->cell[0]->cell[i])->num;
    }
  }
...
lval *builtin_lamdba(lenv *e, lval *a) {
...
lval *lval_call(lenv *e, lval *f, lval *a) {
  ...
  /* While arguments still remain to be processes */
  while (a->count) {
...
void lenv_add_builtins(lenv *e) {
  ...
  lenv_add_builtin(e, "\\", builtin_lamdba);
...
int main(int argc, char **argv) {
  ...
  Expr = mpc_new("expr");
  Byol = mpc_new("byol");
  Comment = mpc_new("string");
  String = mpc_new("comment");
  ...
  mpca_lang(MPCA_LANG_DEFAULT,
            "                                                     \
              ...
              byol     : /^/ <expr>* /$/ ;                        \
           ",
            Number, Symbol, String, Comment, Sexpr, Qexpr, Expr, Byol);
  ...
  mpc_cleanup(8, Number, Symbol, String, Comment, Sexpr, Qexpr, Expr, Byol);
